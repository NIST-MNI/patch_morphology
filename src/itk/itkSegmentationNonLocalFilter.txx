...
---------------------------------------------------------------------------- */
#ifndef __mincSegmentationNonLocalFilter_txx
#define __mincSegmentationNonLocalFilter_txx
...
::BeforeThreadedGenerateData(void)
{
  ...
  {
    m_ProbabilityMaps.resize(m_Process->GetLabelCount());
    for(int i=0;i<m_Process->GetLabelCount();i++)
    {
    ...
    if (m_ProbabilityMaps.size()!=m_Process->GetLabelCount())
    {
      itk::DataObjectError e(__FILE__,__LINE__);
...
#endif
{
  ...
  std::vector<itk::ImageRegionIterator<FloatImageType> > prob_iter(m_Process->GetLabelCount());
  ...
  for (fit = faceList.begin() ; fit != faceList.end(); ++fit)
    ...
    for(int l=0;l<m_Process->GetLabelCount();l++)
    {
    ...
    while ( ! o_iter.IsAtEnd() )
    {
          ...
          if( *kernel_it > itk::NumericTraits<KernelPixelType>::Zero )
          {
            ...
            label_distance.push_back( 
             LabelDistance<LabelPixelType,ImageDimension>(
                ...
                sample,
                search_iterators[sample].GetOffset(i),
                sample_grading
              ) );
          }
        }
      ...
      m_Process->process(label_distance, output, confidence, search_distance, grading, prob);
...
template<class TFeatureImage,class TLabelImage, class TOutputImage, class TSearch,class TPatch,class SampleProcess,class TDistance,class TPreselectionFilter,class TRealType>
void SegmentationNonLocalFilter<TFeatureImage,TLabelImage, TOutputImage, TSearch, TPatch,SampleProcess, TDistance,TPreselectionFilter,TRealType>
